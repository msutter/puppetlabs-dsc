...
Puppet::Type.newtype(:dsc_xdnsserversecondaryzone) do
  ...
  def dscmeta_resource_friendly_name; 'xDnsServerSecondaryZone' end
  def dscmeta_resource_name; 'MSFT_xDnsServerSecondaryZone' end
  ...
  newparam(:dsc_name) do
    ...
    desc "Name - Name of the secondary zone"
    isrequired
    validate do |value|
  ...
  newparam(:dsc_masterservers, :array_matching => :all) do
    def mof_type; 'string[]' end
    def mof_is_embedded?; false end
    desc "MasterServers - IP address or DNS name of the secondary DNS servers"
    validate do |value|
  ...
  newparam(:dsc_type) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "Type - Type of the DNS server zone"
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
