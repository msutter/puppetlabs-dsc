...
Puppet::Type.newtype(:dsc_xadorganizationalunit) do
  ...
  newparam(:dsc_path) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "Path - Specifies the X500 path of the OU or container where the new object is created"
    isrequired
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
  ...
  newparam(:dsc_credential) do
    def mof_type; 'MSFT_Credential' end
    def mof_is_embedded?; true end
    desc "Credential - The credential to be used to perform the operation on Active Directory"
    validate do |value|
      unless value.kind_of?(Hash)
        fail("Invalid value '#{value}'. Should be a hash")
      end
  ...
  # IsMandatory:  False
  # Values:       None
  newparam(:dsc_protectedfromaccidentaldeletion) do
    def mof_type; 'boolean' end
    def mof_is_embedded?; false end
    desc "ProtectedFromAccidentalDeletion - Defaults to True"
    validate do |value|
    end
  ...
  newparam(:dsc_description) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "Description - The description of the OU"
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
